/**
 * DimensionModeToggle — 單一篩選維度的「角色」三選一:不限 ｜ 必須是(AND) ｜ 可以是(OR)。
 ...
 * 三段**皆永遠可按**(不再因空狀態停用):切到必須是/可以是時若沒值,由呼叫端自動選第一項或還原上次。
 *
 * 視覺(DESIGN §5 / Two-Zone 品牌區):選中的必須是/可以是 = Turf Green 翻黑字(與選取 chip 同一
 * 「開著」語言、醒目);不限選中 = 中性灰(「關閉」態,安靜)。
            ...
            className={[
              ...
              active
                ? opt.key === 'none'
                  ...
                  : 'bg-turf-green text-ink-900' // 必須是/可以是選中:草綠翻黑字(醒目,同 chip 語言)
